...
<table class="table">
<tr><th>ID</th><th>Name</th><th>E-Mail</th><th>Mob</th><th colspan="2">Action</th></tr>
<%
...
%>
<tr><td><%=user.getId() %></td><td><%=user.getFname()+" "+user.getSname() %></td><td><%=user.getEmail() %></td><td><%=user.getMob() %></td><td><a href="acceotRequest?sender_id=<%=user.getId()%>&receiver_id=<%=session.getAttribute("id") %>" class="btn btn-success">Accept</a></td><td><a href="rejectRequest?sender_id=<%=user.getId()%>&receiver_id=<%=session.getAttribute("id") %>" class="btn btn-danger">Reject</a></td></tr>
<%} %>
</table>
</div>
